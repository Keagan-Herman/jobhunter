    ...
    >
      ...
      <div className="flex gap-2 flex-wrap mb-6 relative z-10">
        {(job.stack || []).slice(0, 4).map(s => (
          <span key={s} className="text-[9px] px-3 py-1.5 rounded-xl bg-white/[0.03] text-[#7b61ff] font-mono font-black tracking-tight uppercase border border-white/5 transition-all duration-300 group-hover:border-[#7b61ff]/30 group-hover:bg-[#7b61ff]/5">
            {s}
          </span>
        ))}
        {(job.stack || []).length > 4 && (
          <span className="text-[9px] px-3 py-1.5 text-[#555] font-mono font-black bg-white/5 rounded-xl border border-white/5 group-hover:text-[#777]">
            +{(job.stack || []).length - 4}
          </span>
        )}
      </div>
      ...
      <div className="flex justify-between items-center mt-7 pt-5 border-t border-white/5 relative z-10">
        {job.score_reason && (
          <div className="text-[11px] text-[#555] font-medium italic flex-1 mr-4 line-clamp-1 group-hover:text-[#888] transition-colors leading-relaxed">
            &quot;{job.score_reason}&quot;
          </div>
        )}
        <div className="shrink-0 transform transition-all duration-500 group-hover:translate-x-1 group-hover:scale-105">
