  ...
  const renderOverview = () => (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-forwards">
      <div className="space-y-5">
        ...
        {job.url && (
          <a href={job.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-[#00ff87]/5 border border-[#00ff87]/10 text-[11px] text-[#00ff87] font-mono font-black no-underline hover:bg-[#00ff87]/10 hover:border-[#00ff87]/30 transition-all group/link uppercase tracking-widest shadow-lg shadow-[#00ff87]/5">
            Explore Original Listing <span className="group-hover/link:translate-x-1 transition-transform">→</span>
          </a>
        )}
      </div>

      {job.score_reason && (
        <div className="p-8 rounded-[2.5rem] bg-gradient-to-br from-[#00ff87]/[0.08] to-transparent border border-[#00ff87]/20 space-y-4 relative overflow-hidden shadow-[inset_0_0_40px_rgba(0,255,135,0.05)] group/ai">
          <div className="absolute top-0 left-0 w-2 h-full bg-gradient-to-b from-[#00ff87] to-[#00d4ff] opacity-40" />
          <div className="absolute -right-10 -top-10 w-40 h-40 bg-[#00ff87]/10 rounded-full blur-3xl opacity-0 group-hover/ai:opacity-100 transition-opacity duration-1000" />
          <h4 className="text-[10px] font-mono font-black text-[#00ff87] tracking-[4px] uppercase flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#00ff87] animate-pulse" />
            AI Fit Analysis
          </h4>
          <p className="text-[16px] text-white/90 leading-relaxed font-syne font-bold italic">&quot;{job.score_reason}&quot;</p>
        </div>
      )}

      <div className="grid grid-cols-2 gap-5">
        <div className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 group hover:border-white/10 transition-all duration-500 shadow-xl hover:-translate-y-1">
          <div className="text-[10px] font-mono text-[#555] uppercase tracking-[3px] mb-2.5 font-black">Seniority</div>
          <div className="text-[18px] font-extrabold text-white capitalize font-syne tracking-tight">{job.seniority || 'Mid'}</div>
        </div>
        <div className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 group hover:border-white/10 transition-all duration-500 shadow-xl hover:-translate-y-1">
          <div className="text-[10px] font-mono text-[#555] uppercase tracking-[3px] mb-2.5 font-black">Work Style</div>
          <div className="text-[18px] font-extrabold text-white capitalize font-syne tracking-tight">{job.work_style || 'Not specified'}</div>
        </div>
      </div>

      {job.culture_fit && (
        <div className="space-y-5">
          <h4 className="text-[10px] font-mono font-black text-[#555] tracking-[4px] uppercase">Culture Fit</h4>
          <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all duration-500 shadow-2xl relative overflow-hidden group/culture">
            <div className="absolute inset-0 bg-gradient-to-br from-white/[0.02] to-transparent opacity-0 group-hover/culture:opacity-100 transition-opacity duration-700" />
            <p className="text-[15px] text-white/60 leading-relaxed italic relative z-10">&quot;{job.culture_fit}&quot;</p>
          </div>
        </div>
      )}
      ...
      {job.interview_prep && (
        <div className="space-y-5 pb-8">
          <h4 className="text-[10px] font-mono font-black text-[#555] tracking-[4px] uppercase">Interview Strategy</h4>
          <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all duration-500 shadow-2xl relative overflow-hidden group/prep">
             <div className="absolute inset-0 bg-gradient-to-tr from-[#7b61ff]/[0.03] to-transparent opacity-0 group-hover/prep:opacity-100 transition-opacity duration-700" />
            <div className="text-[15px] text-white/60 leading-[1.8] whitespace-pre-wrap font-sans relative z-10">
              {job.interview_prep.split('\n').map((line, i) => (
                <div key={i} className="mb-4 last:mb-0 flex gap-3">
                    <span className="text-[#7b61ff] font-mono font-black">•</span>
                    <span>{line}</span>
                </div>
              ))}
            </div>
